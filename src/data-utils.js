...
export let dataTypeMap = {
  //move defaults to snappyshot.config.js
  number: 2000,
...
let typeAnnotationsMap = {
  //should I just ditch react doc gen and do everything with these types?
...
export function getGenericTypeObject(typeAlias, typeAliases) {
  return typeAlias.right.properties.reduce(
...
export function getPlaceholderFromType(typeAnnotation, typeAliases) {
  if (
    ...
    const typeAlias = typeAliases.find(
      typeAlias =>
        typeAliasExistsForAnnotation(typeAlias, typeAnnotation.id.name)
    );
    if (!typeAlias) {
      return false;
    }
...
export function generateSignaturesFromFlowType(
  params: Array<Object>,
  typeAliases: Array<Object>
) {
  return params
    .filter(
