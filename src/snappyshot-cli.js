...
async function parseFiles(err, files: Array<string>) {
    files.forEach(async filePath => {
        let [fileReadErr, fileSrc] = await to(
            readFilePromise(filePath, { encoding: "UTF-8" })
        )
        if (fileReadErr) {
        ...
        let [writeFileErr, writeFile] = await to(
            toPromise(fs.writeFile)(writeFilePath, snapshot)
        )
        if (writeFileErr || !writeFile) {
